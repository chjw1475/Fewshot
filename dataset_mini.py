...
class dataset_mini(object):
    def __init__(self, n_examples, n_episodes, split, args):
        ...
        self.seed       = args.seed
        self.root_dir   = './dataset/miniImagenet'
...
class dataset_mini_class(Dataset):
    def __init__(self, n_examples, n_episodes, split, args):
        self.im_width, self.im_height, self.channels = list(map(int, args.x_dim.split(',')))
        self.n_examples = n_examples
        self.n_episodes = n_episodes
        ...
        self.train = {'train' : True, 'infer' : False}[args.command]
    ...
    def __getitem__(self, item):
        """
                    ...
                    get unlabel data from n_distractor
                """
        support = np.zeros([self.n_way, self.n_shot, self.im_height, self.im_width, self.channels], dtype=np.float32)
        query = np.zeros([self.n_way, self.n_query, self.im_height, self.im_width, self.channels], dtype=np.float32)
        if self.n_unlabel > 0:
            unlabel = np.zeros([self.n_way + self.n_distractor, self.n_unlabel, self.im_height, self.im_width, self.channels],
                               dtype=np.float32)
        else:
            unlabel = []
            n_distractor = 0

        selected_classes = np.random.permutation(self.n_classes)[:self.n_way + self.n_distractor]
        for i, cls in enumerate(selected_classes[0:self.n_way]):  # train way
            # labled data
            idx1 = np.random.permutation(self.n_label)[:self.n_shot + self.n_query]
            support[i] = self.dataset_l[cls, idx1[:self.n_shot]]
            query[i] = self.dataset_l[cls, idx1[self.n_shot:]]
            # unlabel
            if self.n_unlabel > 0:
                idx2 = np.random.permutation(self.n_unlabel)[:self.n_unlabel]
                unlabel[i] = self.dataset_u[cls, idx2]

        for j, cls in enumerate(selected_classes[self.n_classes:]):  # distractor way
            idx3 = np.random.permutation(self.n_unlabel)[:self.n_unlabel]
            unlabel[i + j] = self.dataset_u[cls, idx3]

        support_labels = np.tile(np.arange(self.n_way)[:, np.newaxis], (1, self.n_shot)).astype(np.uint8)
        query_labels = np.tile(np.arange(self.n_way)[:, np.newaxis], (1, self.n_query)).astype(np.uint8)
        # unlabel_labels = np.tile(np.arange(n_way+n_distractor)[:, np.newaxis], (1, num_unlabel)).astype(np.uint8)
